...
void registermsg(zmsg_t* msg,string dir){
	crearmapa(); //creo tabal hash con todoas las partes de cada cancion
...
void msgSolicitud(zctx_t* context,string addressrecibir,string song){
  ...
  for ( auto it = listP.begin(); it != listP.end(); ++it ){
    ...
    string selectedNode=nodes[random]; //saco una direccion en la cual tengo que hacer a solicitud
...
void handleSolicitudMessage(zmsg_t* msg,zctx_t* context){
  ...
  if(!chunk) {
    ...
    //zmsg_prepend(transferir,&copy);
    sleep(4);
    zmsg_send(&transferir,enviar);
    }
    //zsocket_disconnect (enviar,addressrecibir.c_str());
    //zsocket_destroy (context, enviar);
}
...
void pruebaplay(){
  system("mpg123 ./music/Amy.mp3");
}

//NO ESTOY USANDO espTracker_enviarSolicitud
//Funcion que sirve para crear un hilo que escuche siempre cuando hay una respuesta del tracker
void respTracker_enviarSolicitud(void* tracker,zctx_t *context,string addressRecibir){
  zmq_pollitem_t items[] = {{tracker, 0, ZMQ_POLLIN, 0}};
  cout << "Listening! Hilo:respTracker_enviarSolicitud" << endl;
  while (true) {
    zmq_poll(items, 1, 10 * ZMQ_POLL_MSEC);
    if (items[0].revents & ZMQ_POLLIN) {
      cerr << "From tracker[Respuesta query]\n";
...
int main(int argc, char** argv){
	//NECESITO UN COMANDO PARA DESCONECTARME  zsocket_disconnect
  ...
  string addressSolicitudes="tcp://"+string(argv[3])+":"+string(argv[5]);
	...
	//zsocket_connect(enviar, "tcp://localhost:unpuerto");

	
	//zsocket_connect(enviarSolicitudes, "tcp://localhost:unpuerto");
  //thread t1(pruebaplay);
  ...
  while(true){
    ...
    switch(op){
      ...
      case 3:
      {
        ...
        //ENVIAR MENSAJE AL TRACKER INFORMANDO QUE QUIERE DESCONECTARSE
        zmsg_t* payan=zmsg_new();
        zmsg_addstr(payan,"desconectar");
        zmsg_send(&payan,tracker);
        zmsg_t* respd=zmsg_recv(tracker);
        string desc(zmsg_popstr(respd));
        if(desc.compare("OK")==0){
            zsocket_disconnect(tracker,cadtracker.c_str());
            sleep(1);
            exit(0);
          }
        break;
  ...
  /*while(true){
    string song;
    sleep(2);
    cout<<"Por favor ingrese la cancion: ";
    cin>>song;
    if(songSearch.count(song)>0){
      cout<<"La cancion se encuentra en este nodo"<<endl;
    }else{
      cout<<"La cancion no se encuentra en este nodo, se debe descargar desde otros nodos"<<endl;
      zmsg_t* msgquery=zmsg_new();
      zmsg_addstr(msgquery,"query");
      zmsg_addstr(msgquery,song.c_str());
      zmsg_send(&msgquery,tracker);
	...
	/*system("split -b 256k Amy.mp3 s.");
	system("cat cortado* > ./amy/Amy.mp3");
	*/
	return 0;
}
